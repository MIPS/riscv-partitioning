...
** The Zmwidns extension - addition of `mwidseclist` and its behavior. (See
   <<zmwidns>>.)
** The Zmwidnsc extension - enforcement of sane relationships between the NS-Req
...
The Zmwidns extension adds the `mwidseclist` CSR that is like the WID bitmask
...
reject a load/store/fetch transaction with the Zmwidnsc extension enabled. See
the RISC-V "Worlds" specification for more information on how rejections are
handled.
