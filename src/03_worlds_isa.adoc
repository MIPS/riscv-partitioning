...
** It is reasonable to have WG permanently enabled because default settings can
   ...
   disabled.
** As such, we will document the architecture as it exists with all extensions
   fully implemented and enabled, after which we will define the extensions as
   increasing subsets of that final functionality.
...
introduced by the Hypervisor extension.

Please note that the nested checking, whereby a `mwiddeleg` check is followed
by a `mwidlist` check, is the reason that we don't have to insist that
`mwiddeleg` be a subset of `mwidlist`, and the reason why we can avoid "illegal
value" semantics.

Also worthy of note is the use of an "RoT" privilege level, as discussed in
<<rotMode>>. This might be an external agent or it might be early boot M-mode,
the point is that it controls _lockable_ configuration, that, once locked,
constrains run-time M-mode.
...
As was described in this chapter's preamble, we have so far described the fully
implemented and enabled functionality of the World-ID ISA extension. It remains
for us to describe the sequence of intermediate extensions that arrive at this
final state. We describe the extensions using the same visual language as was
used in <<nonHypervisorTree>> and <<hypervisor>>.
...
The diagram in <<nonHypervisorTree>> is the one that applies here.

==== Smwidv

Dependent on Smwidd, this extension adds the `hlwid`, `hwiddeleg`, and `vslwid`
registers, allowing the World-ID extension to support the adaptations mentioned
in <<hypervisor>> whenever the Hypervisor extension is enabled. When the
Hypervisor extension isn't enabled, this extension has no effect.

The diagram in <<hypervisor>> is the one that applies here.
