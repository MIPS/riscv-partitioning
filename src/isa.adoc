...
** Power-on defaults are given that provide sane system defaults for systems
   wishing to function without any partitioning.
...
* ePMP will add the TrustZone `NS-Attr` to the list of memory attributes it
  considers, whether that is sourced from the platform's PMA infrastructure or
  recorded in the ePMP's own configuration.
...
privilege levels) are allowed to issue transactions, and so if those registers
