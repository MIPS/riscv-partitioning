...
Specification (v0.8.2), upon which this section is based. It specifies a firewall
...
IOPMP implements a global output WID override with the `HWCFG3.rrid_transl`
...
Recall that IOPMP has a register field `HWCFG2.non_prio_en` that indicates
whether the IOPMP instance implements non-priority entries. This is a feature
to support IOPMP instances with large table sizes, where strict ordering
guarantees can be relaxed to allow for improved caching and parallelism within the
hardware. If the IOPMP checker is performing a lookup on the IOPMP entry array
for a given address range and it matches on one or more priority entries, the
IOPMP guarantees to adhere to the criteria and actions specified only in the
highest-priority of those entries (i.e. on the lowest-index match). Many
IOPMP instances will only have priority entries, so they behave in this manner.

However, if a given address range does not match on any of the priority
entries, and if that IOPMP instance implements non-priority entries, then a
match against multiple non-priority entries becomes possible. The IOPMP
specification details the behavior in this case, such that if an address range
matches on multiple non-prioritized entries (e.g. with different access modes
specified in each), the result acts as the “union” of all the matching entries.

The addition of a WID override to the IOPMP entries does not lend itself to any
canonical interpretation of a "union", thus raising the question of how the
device should behave if there are multiple matching, non-prioritized entries
with their own WID overrides. This could be classified as an illegal
(mis)configuration, or some other interpretation might be made in such a case.

****
eIOPMP implementations that implement non-prioritized entries and matching MUST
specify the behavior of match-specific WID overrides in the case where multiple,
non-prioritized entries match on the same transaction.
****
...
Chapter 4 of v0.8.2 of the IOPMP specification contains a summary table of device
...
| `0x0014` | HWCFG3 | No change
| `0x0018` | *HWCFG4* | *New register*
3+^| [...]
...
It should be noted that a new register field, `HWCFG4.prog`, implements a sticky
...
| prog | `1:1` | W1SS | IMP
| Lock-until-reset semantics of this bit apply to the whole eIOPMP configuration,
with the exception of the "Error Capture Registers".
