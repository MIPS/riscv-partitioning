...
****
eIOPMP SHOULD be configurable to allow the NS-Req field to be determined either
(a) as an input field (alongside the RRID/WID input parameter), or (b) via the
`widseclist` device register (derived from the RRID/WID input parameter).
****
...
optional solution is to record the NS-Attr field in the `pmpnsX` registers of
