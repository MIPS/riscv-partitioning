...
* Worlds, a new RISC-V concept that owes its heritage to the WorldGuard definition
  released by SiFive, refined by the RVI security community and in the process of
  ratification.
...
the value 1 corresponds to "Non-Secure", which explains the presence of "NS" in
the definition. When the core initiates a load/store/fetch transaction, this
security level can be signalled on the outgoing request, which explains the
presence of "Req" in the definition.
...
Though the baseline IOPMP definition provides for the implementation of
WID-aware firewalls, it is not sufficient for implementing WID-aware routers.
Consider the following diagram;

image::IOPMPsubsystems.drawio.svg[IOPMPsubsystems,align="center"]

There are firewalls ("checker" and "chk") located both source-side and
target-side, and at both the host scope and within subsystems, and they can be
satisfied by the baseline IOPMP definition. However, the "router" elements are
also required in general.

Additionally, to maintain consistency with the ISA support for NS-Req and
NS-Attr, enhancements are required to the baseline IOPMP definition in order to
provide similar controls at the system level.

__We achieve the aforementioned requirements by a small series of improvements
to IOPMP.__ The resulting "enhanced IOPMP" (hereafter "eIOPMP") can be
instantiated in all the required configurations and use-cases, which will be
spelled out in more detail in <<eiopmp>>.
...
to its configuration interface (otherwise the look-aside is presumably embedded
...
* "Worlds" baseline
** The draft "Worlds" ISA extension set, and the proposed draft to extend
   "Worlds" support to include the Hypervisor extension's privilege levels.
...
.7+| World-ID baseline | Smwid | Hart-specific WID
                       | Smlwid | Lower-than-M-mode WID
                       | Smlwidlist | Constraining lower-than-M-mode WID
                       | Smwdeleg | Delegation
                       | Sswid | Lower-than-S-mode WID
                       | Shlwid | Hypervisor support
                       | Shwdeleg| Hypervisor support
.2+| NS-Req            | Zmwidns | NS-Req assignments and signalling
                       | Zmwidnsc | NS-Req privilege constraints
| NS-Attr              | Zmwidnsa | NS-Attr transaction checking
| PMP registration of NS-Attr | Zmwidnspmp | ePMP registration of NS-Attr
|===
