...
register. These extend the existing WID and WID-delegation concepts to the
additional level of privilege hierarchy that the H extension introduces.

****
Ruud: please check the previous paragraph
****
...
(`mlwid`, `mwiddeleg`, `hlwid`, `hwiddeleg`, `slwid`, `vslwid`). But first a
note about "illegal values".
...
* `vslwid`: __Ruud: please fill in here__
...
| `vslwid` | RW for M | ??? | `0x0` | __Ruud: what do we call it?__
|===
...
==== `vslwid`: __Ruud: what do we call it?__

[%header,cols="1,1,5"]
|===
| Name | Bits | Description
| WID | `[XLEN-1]:0`
| Ruud: one-sentence description of this WID. Only Ceil(Log~2~(NWorlds)) LSBs
are used, others are read-only zero.
|===

****
Ruud: need to describe any relevant semantics here.
****
