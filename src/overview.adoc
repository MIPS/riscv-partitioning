...
for the Hypervisor extension and the new privilege level distinctions that it
introduces. __We will augment the WG definition to support the Hypervisor
extension.__
...
state. __We will define an "NS-Attr" PMA (Physical Memory Attribute) and
associated behavior that can be implemented by the processor - likely in the
ePMP - to perform NS-aware source-side checks in a manner similar to the TZ
model.__
...
the Non-Secure state from accessing a resource marked as Secure.__
